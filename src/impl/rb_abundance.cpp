...
Distance cal_depth_dist2(Normal &p1, Normal &p2) {
...
double cal_depth_dist(size_t r1, size_t r2, size_t i, bool &nz) {
  double d = 0;
  StoredDistance m1 = depth_matrix(r1, i);
  StoredDistance m2 = depth_matrix(r2, i);
  if (m1 > minCV || m2 > minCV) {
    nz = true;
    m1 = std::max(m1, (StoredDistance)1e-6);
    m2 = std::max(m2, (StoredDistance)1e-6);
    if (m1 != m2) {
      StoredDistance v1 = depth_var_matrix(r1, i) < 1 ? 1 : depth_var_matrix(r1, i);
      StoredDistance v2 = depth_var_matrix(r2, i) < 1 ? 1 : depth_var_matrix(r2, i);
      Normal p1(m1, SQRT(v1)), p2(m2, SQRT(v2));
      d = cal_depth_dist2(p1, p2);
    }
  }
...
// O(nobs · num_depth_samples), run once; lets is_nz() avoid re-scanning per pair.
static void build_anynz_cache() {
  if (depth_file.empty()) return;
  g_anynz.assign(nobs, 0);
#pragma omp parallel for schedule(static) num_threads(numThreads)
  for (size_t r = 0; r < nobs; ++r) {
    uint8_t any = 0;
    for (size_t i = 0; i < num_depth_samples; ++i)
      if (depth_matrix(r, i) > minCV) { any = 1; break; }
    g_anynz[r] = any;
  }
}

bool is_nz(size_t r1, size_t r2) {
  if (depth_file.empty()) return true;
  // Fast path: precomputed per-contig flags (built before the graph loop).
  if (g_anynz.size() == nobs) return g_anynz[r1] || g_anynz[r2];
  for (size_t i = 0; i < num_depth_samples; ++i) {
    if (depth_matrix(r1, i) > minCV || depth_matrix(r2, i) > minCV) return true;
  }
  return false;
}

template <typename D>
double _cal_depth_corr(size_t r1, size_t r2, const Matrix &depth1,
                     const Matrix &depth2) {
  assert(num_depth_samples > 1);
...
static std::string rb_bins_info_path() {
  return outFile + ".bins.tsv";
}
static std::string rb_members_path() {
  return outFile + ".members.tsv";
}
static std::string rb_matrix_path() {
  return outFile + ".members.matrix.tsv";
}
