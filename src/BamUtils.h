...
int bam_fetch(const BamFile &bf, const hts_idx_t *idx, int tid, int beg,
              int end, void *data, bam_fetch_f bff);
...
public:
  ...
  BamCache(int maxSize = 32) { cached.reserve(maxSize); }
  ...
  ~BamCache() { clear(); }
  void swap(BamCache &other) { cached.swap(other.cached); }
  void clear() {
    for (BamVector::iterator it = cached.begin(); it != cached.end(); it++) {
      // cerr << "BamCache::clear() destroy: " << bam1_qname(*it) << "," <<
      // (*it)->core.flag << " " << (long) *it << endl;
      if (*it)
        bam_destroy1(*it);
      *it = NULL;
    }
  ...
  void putBam(bam1_t *bam) {
    if (bam != NULL) {
      if (cached.size() < cached.capacity()) {
        ...
        // cerr << "BamCache::putBam() destroy : " << (long) bam << " " <<
        // ((bam->m_data > 0) ? bam1_qname(bam) : "undef") << "," <<
        // bam->core.flag << " " << endl;
        bam_destroy1(bam);
      }
    }
...
class BamFile {
  // Helper class to manage memory of Bam, index and headers.  Safe for
  // containers
public: // Objectify bam pointer types
#ifdef LEGACY_SAMTOOLS
  static void SamFileCloser(samfile_t *sam) {
    sam->header = NULL;
    samclose(sam);
  }
  static void BamHeaderDestroyer(bam_header_t *header) {
    bam_header_destroy(header);
  }
  static void BamIndexDestroyer(bam_index_t *index) {
...
public:
  BamFile() : start(0) {}
  BamFile(string _filePath, bool loadIdx = true)
      : filePath(_filePath), start(0) {
    try {
      open(loadIdx);
    } catch (std::bad_alloc e) {
      cerr << "Could not allocate enough memory to open BAM: " << filePath
           << (loadIdx ? " and .bai " : " ") << endl;
      throw e;
    }
  }
...
#ifdef LEGACY_SAMTOOLS
  bamFile getBamFile() { return samfile->x.bam; }
#else
  BamFile &getBamFile() { return *this; }
#endif
  operator samfile_t *() { return samfile.get(); }
  string getFilePath() const { return filePath; }
  string getBamName() const { return bamName; }
  int64_t getDataStart() const { return start; }
  BamCache &getBamCache() { return bamCache; }
...
inline void warnMalformedBamRead(const bam1_t *b, const std::string &msg) {
  static std::atomic<int> warningCount{0};
  constexpr int maxWarnings = 25;
  int warningIdx = warningCount.fetch_add(1, std::memory_order_relaxed);
  if (warningIdx >= maxWarnings) {
    if (warningIdx == maxWarnings) {
#pragma omp critical(OUTPUT)
      {
        std::cerr << "WARNING: additional malformed BAM record messages "
                     "suppressed after "
                  << maxWarnings << " warnings" << std::endl;
      }
    }
    return;
  }
#pragma omp critical(OUTPUT)
  {
    std::cerr << "WARNING: malformed BAM record";
    if (b != NULL) {
      std::cerr << " '" << bam1_qname(b) << "'";
      std::cerr << " tid=" << b->core.tid << " pos=" << b->core.pos;
    }
    std::cerr << ": " << msg << std::endl;
  }
}

inline bool expectMalformedBamInvariant(const bam1_t *b, bool condition,
                                        const std::string &msg) {
  if (!condition) {
    warnMalformedBamRead(b, msg);
  }
  return condition;
}
...
class NameBamMap {
  ...
  // upon erase, a hash of the name is remembered so further insertions of the
  // same name will not occur bam1_t are duplicated and destroyed on insert &
  // erase respectively
public:
  ...
  NameBamMap(BamNameTrackingChooser *_trackName = NULL)
      : bamCache(32 * 1024), trackName(_trackName) {}
  ...
  iterator insert(string name, const bam1_t *bam, bool forceInsert = false) {
    ...
    if (existing != map.end() && !forceInsert) {
      ...
      if (existing != map.end()) {
        // cerr << "Replacing " << bam1_qname(existing->second) << "," <<
        // existing->second->core.flag << " with " << bam1_qname(bam) << "," <<
        // bam->core.flag << endl;
        ...
        if (trackName == NULL || !trackName->unsupportedRead(bam)) {
          ...
          // cerr << "Storing " << bam1_qname(dup) << "," << dup->core.flag << "
          // " << ((long) dup) << endl;
  ...
  void erase(iterator it) {
    ...
    if (trackName == NULL ||
        ((*trackName)(bam) && !trackName->unsupportedRead(bam))) {
      // cerr << "Marking " << bam1_qname(bam) << "," << bam->core.flag <<
      // ((long) bam) << endl;
      it->second = NULL;
    } else {
      // cerr << "Erasing " << bam1_qname(bam) << "," << bam->core.flag <<
      // ((long) bam) << endl;
      map.erase(it);
    }
    bamCache.putBam(bam);
  ...
  void erase(string name) { erase(find(name)); }
  void clear() {
    for (iterator it = begin(); it != end(); it++) {
      // cerr << "Clearing " << bam1_qname(it->second) << "," <<
      // it->second->core.flag << " " << ((long) it->second) << endl;
...
public:
  ...
  static bool validateHeader(const BamHeaderT &ref, BamHeaderT &test,
                             string ref_name = string(),
                             string test_name = string()) {
    if (ref->n_targets != test->n_targets) {
      cerr << "ERROR: validateHeader - Header count mismatch ("
           << ref->n_targets << " vs " << test->n_targets
           << ") between bam files " << ref_name << " and " << test_name
           << endl;
      return false;
    }
    for (int32_t i = 0; i < ref->n_targets; i++) {
      if (strcmp(ref->target_name[i], test->target_name[i]) != 0) {
        cerr << "ERROR: validateHeader - sequence name mismatch at " << i
             << " (" << ref->target_name[i] << " vs " << test->target_name[i]
             << ") between bam files" << ref_name << " and " << test_name
             << endl;
        return false;
      }
      if (ref->target_len[i] != test->target_len[i]) {
        cerr << "ERROR: validateHeader - sequence size mismatch at " << i
             << ", " << ref->target_name[i] << " (" << ref->target_len[i]
             << " vs " << test->target_len[i] << ") between bam files"
             << ref_name << " and " << test_name << endl;
        return false;
      }
    }
...
#pragma omp critical(BamFile)
  ...
  static bool validateHeader(const BamFile &src, BamFile &test,
                             bool nullOK = false) {
    ...
    bool is_valid = validateHeader(src.header, test.header, src.getBamName(),
                                   test.getBamName());
  ...
  static BamHeaderT openBamsAndConsolidateHeaders(const StringVector &bamPaths,
                                                  BamFileVector &bams,
                                                  bool loadIdx = true) {
    // open all bam files
...
#pragma omp parallel reduction(+ : validHeaders)
    for (int bamIdx = 0; bamIdx < (int)bamPaths.size(); bamIdx++) {
      ...
      if (bamIdx == 0) {
#pragma omp critical(OUTPUT)
        cerr << "Three are " << bam.header->n_targets
             << " contigs in the first bam file: " << bamPath << endl;
      }
      if (bamIdx > threadNum) {
...
#pragma omp critical(OUTPUT)
          cerr << omp_get_thread_num() << ": ERROR: " << bamPath
               << " has a different reference than " << bamPaths[refIdx]
               << endl;
        }
      }
#pragma omp critical(OUTPUT)
      cerr << omp_get_thread_num() << ": Processed header: " << bamPath;
    }
    // now reduce to just one shared header
#pragma omp parallel for reduction(+ : validHeaders)
    for (int bamIdx = 1; bamIdx < (int)omp_get_max_threads(); bamIdx++) {
      if (bamIdx >= (int)bamPaths.size())
        continue;
      bool isValid = validateHeader(bams[0], bams[bamIdx], true);
      if (isValid) {
        validHeaders += 1;
      } else {
        cerr << "ERROR: " << bamPaths[bamIdx]
             << " has a different reference than " << bamPaths[0] << endl;
      }
    }
    // one to rule them all
    if (validHeaders != bamPaths.size()) {
      cerr << "\n\nERROR: It appears that your bam files contain different "
              "references.\n"
           << "\tvalidHeaders (including the first bamfile) == " << validHeaders
           << " while there were " << bamPaths.size() << " bams to summarize.\n"
           << "\n\n\tPlease ensure that all reads are aligned to the exact "
              "same assembly\n\n"
           << endl;
      exit(1);
    }
    assert(bams[0].header.get() != NULL);
    cerr << "Consolidated header with "
         << bams[0].header->n_targets << " contigs from " << bamPaths.size()
         << " bam files." << endl;
    return bams[0].header;
  }

  static void preprocessContigNames(BamHeaderT header) {
    // exercise the name2tid lookups that get populated while parsing the
    // alignmnts this should reduce the overall memory footprint
  ...
  static ostream &writeFastq(ostream &os, const bam1_t *bam, const bam1_t *bam2,
                             int offset = 33) {
  ...
  static bool writeFastqOrStorePair(ostream &os, const bam1_t *b,
                                    NameBamMap &map,
                                    bool onlyIfPairIsStored = false,
                                    int offset = 33) {
  ...
  static void writePairOrSingle(ostream &osPaired, ostream &osSingle,
                                const bam1_t *b, const bam1_t *mate,
                                int offset = 33, BamCache *bamCache = NULL) {
    if (mate != NULL) {
      writeFastq(osPaired, b, mate, offset);
    } else if (b->core.l_qseq > 70) {
      // no mate was found in the file.  Write to single stream and break pair
      // association
      bam1_t *dup;
      if (bamCache == NULL)
        dup = bam_dup1(b);
      ...
      if (bamCache == NULL) {
        // cerr << "writePairOrSingle() destroy : " << bam1_qname(dup) << "," <<
        // dup->core.flag << " " << (long) dup << endl;
        bam_destroy1(dup);
      } else {
        bamCache->putBam(dup);
      }
...
private:
  ...
  public:
    _findPairInFileData(const bam1_t *_bam, bam1_t *_mate)
        : bam(_bam), name(getBaseName(_bam)), mate(_mate), success(false) {}
    const bam1_t *bam;
    string name;
    bam1_t *mate;
    bool success;
  ...
  static int _findPairInFile(const bam1_t *orphan, void *_data) {
    ...
    if (data->name.compare(orphanName) == 0 &&
        data->bam->core.flag != orphan->core.flag) {
...
public:
  static bool findPairInFile(BamFile &bf, const bam1_t *b, bam1_t *mateMatch,
                             bool resetPosition = false) {
    int64_t current = resetPosition ? bam_tell(bf.getBamFile()) : 0;

    _findPairInFileData data(b, mateMatch);
    bam_fetch(bf.getBamFile(), bf.index.get(), b->core.mtid, b->core.mpos,
              b->core.mpos, &data, _findPairInFile);

    if (resetPosition) {
      int64_t ret = bam_seek(bf.getBamFile(), current, SEEK_SET);
      if (ret != 0)
        cerr << "Warning could not bam_seek to " << current << " for "
             << bf.getFilePath() << endl;
    }
  ...
  static void writeFastqOrphans(ostream &osPaired, ostream &osSingle,
                                NameBamMap &map, BamFile &bf, int offset = 33) {
    if (map.empty())
      return;
    ...
    if (bf.index == NULL || (int)map.size() > estimatedBgzfBlocks / 128) {
      ...
      // re-read the entire file as it will likely be faster than random access
      // using the index
      ...
      while ((bytesRead = SAMREAD(bf, bam)) >= 0) {
        ...
        if (bytesRead == 0)
          break;
#endif
        if ((bam->core.flag & (BAM_FPAIRED | BAM_FUNMAP | BAM_FMUNMAP)) ==
            (BAM_FPAIRED | BAM_FUNMAP | BAM_FMUNMAP))
      ...
      for (it = map.begin(); it != map.end(); it++) {
        ...
        if (it->second->core.l_qseq > 70) {
          ...
          // cerr << "writeFastqOrphans(scan): " << bam1_qname(it->second) <<
          // endl;
        }
      }
      ...
      for (it = map.begin(); it != map.end(); it++) {
        ...
        writePairOrSingle(osPaired, osSingle, bam, foundMate ? mate : NULL,
                          offset, &map.getBamCache());

        // cerr << "writeFastqOrphans(seek): " << bam1_qname(bam) << "," <<
        // bam->core.flag << " " << ((long) bam) << ": mate " << foundMate <<
        // "," << (foundMate ? -1 : mate->core.flag) << endl;
      }
...
private:
  ...
  public:
    _writeFastqByContigData(ostream &_os, NameBamMap &_map, int32_t _tid,
                            int _offset)
...
public:
  static void writeFastqByContig(ostream &os, NameBamMap &map, BamFile &bf,
                                 int32_t tid, bool resetPosition = false,
                                 int offset = 33) {
    ...
    if (bf.index != NULL) {
      bam_fetch(bf.getBamFile(), bf.index.get(), tid, 0,
                bf.header->target_len[tid] - 1, &data, _writeFastqByContig);
    } else {
      ...
      while ((bytesRead = SAMREAD(bf, b)) >= 0) {
        ...
        if (b->core.tid < 0 || b->core.tid > tid) {
          if (prev == current) {
            ...
            int64_t ret =
                bam_seek(bf.getBamFile(), bf.getDataStart(), SEEK_SET);
            if (ret != 0)
              cerr << "Warning could not bam_seek to " << bf.getDataStart()
                   << " for " << bf.getFilePath() << endl;
          } else {
            // rewind to this record
            int64_t ret = bam_seek(bf.getBamFile(), prev, SEEK_SET);
            if (ret != 0)
              cerr << "Warning could not bam_seek to " << prev << " for "
                   << bf.getFilePath() << endl;
            break;
          }
        }
...
// FIXME: This just returns the readgroup... looking in the header is disabled
// for htslib AFAIK
const char *bam_get_library(bam_header_t *h, const bam1_t *b) {
  const uint8_t *rg;
  rg = bam_aux_get(b, "RG");
