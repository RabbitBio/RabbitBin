...
// Linkage-based strain resolution (feature #5, iteration 2).  Per-site allele
// frequencies across samples trace each minor strain's relative abundance; sites
// whose per-sample frequency trajectories agree belong to the same strain, so
// counting coherent trajectory clusters within a bin estimates its strain count
// and recovers each strain's per-sample abundance — the DESMAN/inStrain signal,
// but computed in the same BAM pass and bounded by a per-contig site cap.
static int    g_strain_max_sites = 96;   // --strain-max-sites (per contig,sample)
static int    g_strain_min_sites = 12;   // --strain-min-sites (min sites/bin)
static int    g_strain_max_k     = 4;    // --strain-max-k (cap on reported strains)
// Per (large contig, sample) capped polymorphic sites, row-aligned with
...
static bool   g_cert_core_fasta = false; // --cert-core-fasta: write core_bins/ FASTA
...
// Strain resolution (feature #5) + stability certification (feature #6) live in
// impl/rb_strain.cpp and impl/rb_certify.cpp (kept out of this file to limit its
...
#include "impl/rb_strain.cpp"
#include "impl/rb_certify.cpp"
  ...
  if (ac >= 2) {
    ...
    if (sub == "--help" || sub == "-h") {
      cerr << "RabbitBin " << version << " (" << DATE << ")\n\n"
           ...
           << "       rabbitbin -a contigs.fa -d depth.tsv -o out   # legacy bin form\n\n"
           << "Commands:\n"
           << "  bin     Metagenome binning from FASTA + depth/BAM/CRAM\n"
           << "          Key options: --save-cache/--load-cache (fast re-binning),\n"
           << "          --autotune/--auto (SCG-guided parameter search),\n"
           << "          --qc/--purify/--keep-hq-only (SCG quality + cleanup),\n"
           << "          --taxonomy, --bioboxes, --resolutions, --long-read\n"
           ...
           << "  rabbitbin refine -k contigs.markers.tsv -o consensus \\\n"
           << "                   -i mags.members.tsv -i metabat2.binning\n"
           << "  rabbitbin amber -g gold.binning --members mags.members.tsv\n\n"
