...
public:
  static int &_edgeBases() {
    static int _ = 75;
    return _;
  }
  int edgeBases() const { return std::min(avgRead / 3, _edgeBases()); }
  ...
  bool failedMapping(const bam1_t *b) {
    if ((b->core.flag & (BAM_FUNMAP | BAM_FMUNMAP)) != 0) {
      ...
      if (b->core.tid < 0 || !contigPass[b->core.tid]) {
        ...
        if ((b->core.flag & BAM_FPROPER_PAIR) != BAM_FPROPER_PAIR ||
            b->core.mtid < 0 || !contigPass[b->core.mtid]) {
          return true;
        }
      }
    }
  ...
  bool edgeRead(const bam1_t *b) {
    // check for edge read
    auto edge = edgeBases();
    bool shortContig = (int)header->target_len[b->core.tid] <= edge;
    bool middleRead =
        b->core.pos >= edge &&
        (b->core.pos + edge) < (int)header->target_len[b->core.tid];
  ...
  bool edgeMateRead(const bam1_t *b) {
    if ((b->core.flag & BAM_FPAIRED) == BAM_FPAIRED) {
      ...
      auto edge = edgeBases();
      bool mateShortContig = (int)header->target_len[mtid] <= edge;
      bool mateMiddleRead =
          b->core.mpos >= edge &&
          (b->core.mpos + edge) < (int)header->target_len[mtid];
  ...
  static bam1_t *fixEndClip(bam1_t *b, const bam_header_t *header) {
    if ((b->core.flag & BAM_FUNMAP) == BAM_FUNMAP) {
      ...
      if(bam_validate1(header, b) == 0)
              std::cerr << "Warning new bam starts out invalid! " <<
      bam1_qname(b) << "tid=" << b->core.tid << " data_len=" << b->data_len << "
      l_qname=" << (int) b->core.l_qname << " nullpos=" << (void*)
      &bam1_qname(b)[b->core.l_qname-1] << " mchrpos=" << (void*)
      memchr(bam1_qname(b), '\0', b->core.l_qname) << std::endl;
      */
      if ((uint32_t)b->core.pos >= (uint32_t)header->target_len[b->core.tid] ||
          b->core.pos < 0) {
        std::cerr << "Warning bam starts outside of the reference coords! pos="
                  << b->core.pos << " tid=" << b->core.tid
                  << " target_len=" << header->target_len[b->core.tid]
                  << " tname=" << header->target_name[b->core.tid]
                  << " bam: " << bam1_qname(b) << endl;
        return b;
      }
      bam1_t *newBam = fixEndClip(b, header->target_len[b->core.tid]);
      /* FIXME
      if (bam_validate1(header, newBam) == 0) {
              std::cerr << "Warning new bam is invalid! " << bam1_qname(newBam)
      << std::endl;
      }
      */
      return newBam;
...
protected:
  static bam1_t *fixEndClip(bam1_t *b, uint32_t refLen) {
    ...
    if (alignend > refLen) {
      ...
      for (int i = 0; i < numCigarOperations; i++) {
        ...
        if ((optype & 0x02) == 0x02)
          refpos += oplen;
        if (refpos > refLen) {
          std::cerr << "WARNING: adjusting cigar operation at op " << i << " ("
                    << oplen << bam_cigar_opchr(op)
                    << ") as the alignment refpos=" << refpos
                    << " exceeds the refLen=" << refLen << ".";
          std::cerr << " before: endpos=" << bam_endpos(b)
                    << " cig2qlen=" << bam1_cigar2qlen(b, cigar)
                    << " l_qseq=" << b->core.l_qseq;
          // make this op the last cigar operation then soft clip everything to
          // the end of the sequence
          // TODO if this is a match, allow match to ref end, then add softclip
          // (potentially adding a cigar op)
          if (numCigarOperations >= i + 1) {
            // reduce the number of CIGAR operations and adjust the data
            // structure accordingly: all variable-length data, concatenated;
            // structure: qname-cigar-seq-qual-aux
            std::cerr << " reducing cigar ops from " << numCigarOperations
                      << " to " << i + 1;
            ...
            if (!expectMalformedBamInvariant(
                    b, remainderLen > 0,
                    "cannot clip overhanging alignment because remainderLen "
                    "<= 0")) {
              return b;
            }
            b->core.n_cigar = i + 1;
            ...
            if (!expectMalformedBamInvariant(
                    b, oldSeqStart > newSeqStart,
                    "cannot clip overhanging alignment because cigar "
                    "compaction did not shrink the record")) {
              return b;
            }
            int newDataLen = oldDataLen - (oldSeqStart - newSeqStart);
            if (!expectMalformedBamInvariant(
                    b, newDataLen < oldDataLen,
                    "cannot clip overhanging alignment because the compacted "
                    "record did not get smaller")) {
              return b;
            }
          ...
          if ((optype & 0x01) == 0x01)
            preop_seqpos = seqpos - oplen;
          if (!expectMalformedBamInvariant(
                  b, preop_seqpos <= (uint32_t)b->core.l_qseq,
                  "cannot clip overhanging alignment because the clipped "
                  "query start is beyond the query length")) {
            return b;
          }
          uint32_t newoplen = b->core.l_qseq - preop_seqpos;
          cigar[i] = bam_cigar_gen(newoplen, BAM_CSOFT_CLIP);
          std::cerr << " changed to '" << newoplen << "S' for "
                    << bam1_qname(b);
          std::cerr << " endpos=" << bam_endpos(b) << " refLen=" << refLen
                    << " cig2qlen=" << bam1_cigar2qlen(b, cigar)
                    << " l_qseq=" << b->core.l_qseq;
          std::cerr << std::endl;
          int newEnd = bam_endpos(b);
          if (!expectMalformedBamInvariant(
                  b, newEnd - skipRef <= refLen,
                  "alignment still extends past the reference after "
                  "soft-clipping the end")) {
            return b;
          }
          if (!expectMalformedBamInvariant(
                  b, bam1_cigar2qlen(b, cigar) == b->core.l_qseq,
                  "soft-clipped alignment has a query length that no longer "
                  "matches the BAM record")) {
            return b;
          }
          break;
        }
    ...
    bool endOkay = checkEnd(b, refLen);
#ifndef NDEBUG
    assert(endOkay);
#endif
    if (!endOkay) {
